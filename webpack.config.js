...
var webpackOptions = {
  ...
  entry: {
    ...
    common: ["core-js/es6/promise", "dom4", "vue", "vue-router"].concat(
      commonLib
    )
  },
...
if (isProd) {
  webpackOptions.plugins.push(new webpack.HashedModuleIdsPlugin());
  ...
  if (config.prerenderPaths && config.prerenderPaths.length) {
    ...
    webpackOptions.plugins.push(
      new PrerenderSpaPlugin(__dirname + "/dist", config.prerenderPaths)
    );
  }
} else {
